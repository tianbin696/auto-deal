...
class TushareAPI:
    ...
    def get_lianban_list(self, timeStr=None, index=0, do_cache=False):
        codes = []
        index_df = self.get_index_h_data("399001.SZ", do_cache=do_cache)
        if index_df['close'][index] < numpy.min(index_df['close'][index+1:index+31])*0.99:
            return codes
        ...
        for code in list(open(path)):
            ...
            try:
                ...
                if len(df['close']) <= index:
                    continue
                if max(max(df['open'][index], df['close'][index+1])*1.03, df['high'][index]*0.95, numpy.max(df['close'][index:index+60])*0.80) \
                        <= df['close'][index] <= numpy.min(df['close'][index:index+60])*1.20 \
