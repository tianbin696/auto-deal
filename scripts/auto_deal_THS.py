    ...
    def loopMonitor(self):
        ...
        while True:
            try:
                ...
                if (self.compare("09", "26") and not self.compare("11", "30")) or (self.compare("13", "00") and not self.compare("15", "00")):
                ...
                if self.compare("9", "32") and not deal_started:
                    self.operation.saveScreenshot("开始交易", '9:32 开始交易')
                    deal_started = True
...
def get_direction_by_avg2(code, prices, vols, is_logging=True, open_price=0, highest_price=0):
    ...
    if prices[1]*0.95 < prices[0] < numpy.max(prices[0:60])*0.80:
        if avgs[0] > avgs[1] > avgs[2] > avgs[3] and avgs[3] < avgs[4] < avgs[5] < avgs[6]:
            return 'B'
    if prices[1]*1.05 > prices[0] > numpy.min(prices[0:30])*1.15:
        if avgs[0] < avgs[1] < avgs[2] < avgs[3] and avgs[3] > avgs[4] > avgs[5] < avgs[6]:
            return 'S'
    return 'N'
...
def get_direction_by_avg(code, prices, vols, is_logging=True, open_price=0, highest_price=0):
    ...
    direction = 'N'
    if prices[0] > numpy.min(prices[1:days4]) > 0 and prices[1]*1.06 > prices[0] > max(highest_price*0.97, prices[1]*0.97):
        if diff_1 > 0 > diff_2 and avg1_0 > avg1_1 > avg1_2:
            direction = 'B'
        if diff_1 > diff_2 > 0 > diff_3 and avg1_0 > avg1_1 > avg1_2:
            direction = 'B'
        if numpy.max(prices[1:31])*0.80 > prices[0] and 0 < liang_bi < 0.70:
            direction = 'B'
        if 0 < liang_bi < 0.45:
            direction = 'B'
    if diff_1 < 0 < diff_2 and prices[1]*0.91 < prices[0] < prices[1]:
        direction = 'S'
...
def get_direction_for_lianban(code, prices, vols, is_logging=True, open_price=0, highest_price=0):
    ...
    if avg_1 < avg_2 and prices[0] < min(open_price, prices[1])*0.94:
        return 'S'
    if prices[0] < highest_price*0.92 or prices[0] < min(open_price, prices[1])*0.93:
        return 'S'
    if prices[0] < numpy.min(prices[1:21]) and prices[0] < min(prices[1], open_price)*0.98:
        return 'S'
    return 'B'
