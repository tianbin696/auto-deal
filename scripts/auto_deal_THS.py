    ...
    def getDirection(self, code, price, open_price, highest_price, lowest_price, chen_ben=0, volume=0):
        ...
        if code not in isSelleds or not isSelleds[code]:
            try:
                ...
                if price < max(open_price, avg1)*0.97 and price > numpy.max(df['high'][1:25])*0.8 and volume > min(numpy.mean(df['volume'][1:6]), numpy.mean(df['volume'][1:11])):
                    # 高位放量长阴线，卖出
                    return 'S'
            except Exception as e:
        ...
        if code not in isBuyeds or not isBuyeds[code]:
            if price > min(open_price, avg1)*1.03 and price < numpy.min(df['low'][1:25])*1.2 and volume > min(numpy.mean(df['volume'][1:6]), numpy.mean(df['volume'][1:11])):
                # 低位放量长阳线，买入
                return 'B'
