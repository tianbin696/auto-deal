...
def get_direction_by_avg(code, prices, vols, is_logging=True, open_price=0, highest_price=0):
    ...
    if prices[0] > numpy.min(prices[1:days4]) > 0 and prices[0] > highest_price*0.93:
        ...
        if diff_1 > diff_2 > diff_3 > 0 > diff_4 and prices[0] > prices[1]*0.96:
            direction = 'B'
        if numpy.max(prices[1:2*days4])*0.87 > prices[0] > prices[1]*0.96 and vols[0]/numpy.mean(vols[1:6]) < 0.6:
            direction = 'B'
    if diff_1 < 0 < diff_2:
        direction = 'S'
    if 0 < prices[0] < prices[1]*0.92 or 0 < prices[0] < open_price * 0.92 or 0 < prices[0] < highest_price*0.93:
        direction = 'S'
    if 0 < prices[0] < numpy.min(prices[1:days4]) and vol1 > vol2:
        direction = 'S'
    if numpy.min(prices[1:2*days4])*1.2 < prices[0] < prices[1]*1.04 and vols[0]/numpy.mean(vols[1:6]) > 1.9:
        direction = 'S'
    if is_logging:
