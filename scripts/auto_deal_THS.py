...
class Monitor:
    ...
    def getDirection(self, code, price, open_price, highest_price, lowest_price, chen_ben=0, volume=0):
        ...
        if code not in isBuyeds or not isBuyeds[code]:
            if price > open_price and price > min(open_price, avg1)*1.04 and price < numpy.min(df['low'][1:25])*1.2 and volume > min(numpy.mean(df['volume'][1:6]), numpy.mean(df['volume'][1:11])):
                # 低位放量长阳线，买入
                return 'B'
